...
def backward_locomotion(
    gait_type: str = "walk", total_time: float = 25, use_gui: bool = True
):
    """
    Simulate the backward locomotion of the robot using the CPG data.
    ...
    # 1. Load the CPG data
    # Handle single gait
    file_path = BACKWARD_DATA_DIR / f"cpg_{gait_type}_backward_{total_time}s.csv"
    if not file_path.exists():
        print("Data does not exist. Exporting the data...")
        export_cpg_data(gait_type=gait_type, total_time=total_time, _if_backward=True)
    print(f"Loading the data from: {file_path}")
    df = pandas.read_csv(file_path, header=None)
    cpg_data = df.to_numpy()
    cpg_data = cpg_data[int(10 * 500) :, :]
    ...
    fixed_base = False
    initial_height = 0.5 if fixed_base else 0.26
    ...
    for i in range(len(cpg_data)):
        ...
        processed_data = network._process_data(cpg_state, gait_type)
    ...
    joint_file = (
        BLENDER_DATA_DIR / "backward_locomotion" / f"To_blender_{gait_type}_data.csv"
    )
...
if __name__ == "__main__":
    backward_locomotion(gait_type="walk", total_time=20)
